...
def change_xml_list_annotation(root, image_id, new_target, saveroot, id):
    ...
    elem.text = (str(id) + '.png')
    xmlroot = tree.getroot()
    # 修改增强后的xml文件中的path
    elem = tree.find('path')
    if elem != None:
        elem.text = (saveroot + str(id) + '.png')
...
if __name__ == "__main__":
    ...
    for name in tqdm(os.listdir(XML_DIR), desc='Processing'):
        ...
        og_img.convert('RGB').save(AUG_IMG_DIR + name[:-4] + '.png', 'JPEG')
        og_xml = open(os.path.join(XML_DIR, name))
        tree = ET.parse(og_xml)
        # 修改增强后的xml文件中的filename
        elem = tree.find('filename')
        elem.text = (name[:-4] + '.png')
        ...
        for epoch in range(AUGLOOP):
            ...
            path = os.path.join(AUG_IMG_DIR,
                                str(str(name[:-4]) + '_' + str(epoch)) + '.png')
