...
def train():
    ...
    xIteration = []
    yLoss = []
    f = open('lossdata.csv', 'w', encoding='utf-8', newline='')
    ...
    for iteration in range(args.start_iter, cfg['max_iter']):  # 循环训练
        ...
        if iteration % 10 == 0:
            ...
            csv_writer.writerow([repr(iteration), loss.item()])
        # if args.visdom:
        #     update_vis_plot(iteration, loss_l.data[0], loss_c.data[0],
        #                     iter_plot, epoch_plot, 'append')
        max_iter = int(config.voc['max_iter'])
        if iteration != 0 and iteration % max_iter == 0:
